...
## Z.44 — TrustPhotoBand crop cut the crew's heads at every desktop width

| | |
|---|---|
| **Report** | Owner, with a screenshot: the crew's heads are cut off at the top of the "Real crew, real accountability." band. |
| **Measured, not eyeballed** | Source `about-team-shop-bay.jpg` is 2800x1867 (3:2). The topmost cap in the frame sits at **19.9%** of the image height (measured against the source: 265px of a 1334px render). The band is a short full-bleed strip — measured 478px tall at 1265px wide, 484px at 1905px — so `object-fit: cover` scales the photo to fill the width and discards most of its height. |
| **Root cause** | `objectPosition` was `30% 50%`. Simulating `cover` against the real section height across desktop widths, the visible window began at **22%** (1280px) rising to **39.6%** (3440px) of the image. The head line is 19.9%. So the heads were cut at *every* desktop width, not only wide ones — the vertical centring was never going to include them. |
